...
exports.onNewComment = functions.firestore
    ...
    const commentPreview = ((_a = comment.text) === null || _a === void 0 ? void 0 : _a.substring(0, 100)) || "";
...
// ============================================================
const TIER_REVEAL_RADIUS = {
    1: 15, 2: 15, 3: 15, 4: 15, 5: 30,
    6: 40, 7: 60, 8: 80, 9: 100, 10: 150,
};
function getRevealRadius(tierLevel, isGroup) {
    if (!isGroup)
        return 15; // Regular secrets always 15m
...
function checkVelocity(uid, lat, lng) {
    const now = Date.now();
    ...
    if (elapsed < 1)
        return true; // Too close in time to judge
...
exports.verifyGroupUnlock = functions.https.onCall(async (data, context) => {
    ...
    const rawIp = ((_d = (_c = (_b = (_a = context.rawRequest) === null || _a === void 0 ? void 0 : _a.headers) === null || _b === void 0 ? void 0 : _b["x-forwarded-for"]) === null || _c === void 0 ? void 0 : _c.split(",")[0]) === null || _d === void 0 ? void 0 : _d.trim()) ||
        ((_e = context.rawRequest) === null || _e === void 0 ? void 0 : _e.ip) ||
        "unknown";
    ...
    const uniqueAttempts = recentAttempts.filter((d) => {
        var _a;
        const ip = ((_a = d.data()) === null || _a === void 0 ? void 0 : _a.ip) || "unknown";
        if (uniqueIps.has(ip))
            return false;
        uniqueIps.add(ip);
        return true;
    ...
    if (uniqueAttempts.length >= requiredUsers) {
        const unlockedUids = uniqueAttempts.map((d) => d.id);
    ...
    return {
        success: false,
        message: `Need ${requiredUsers - uniqueAttempts.length} more people within ${timeWindowMinutes} minutes.`,
...
exports.revealSecret = functions.https.onCall(async (data, context) => {
    ...
    if (!allowed) {
        ...
        const savedIds = ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.savedSecretIds) || [];
    ...
    if (!allowed) {
        if (userLat == null || userLng == null) {
            throw new functions.https.HttpsError("invalid-argument", "Location required to reveal this secret");
        }
        const tierLevel = secret.creatorTierLevel || 1;
        ...
        if (dist > revealRadius) {
            throw new functions.https.HttpsError("permission-denied", `Too far (${Math.round(dist)}m). Must be within ${revealRadius}m.`);
        }
        // Group secrets require group unlock flow, not individual reveal
        if (secret.isGroup) {
            throw new functions.https.HttpsError("permission-denied", "Group secret requires group unlock");
        }
        allowed = true;
    }
    if (!allowed) {
...
// ============================================================
exports.createSecretV2 = functions.https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError("unauthenticated", "Login required");
    const uid = context.auth.uid;
    const { type, textContent, audioURL, audioDuration, lat, lng, isGroup, requiredUsers, timeWindowMinutes, } = data;
    ...
    const userDoc = await db.collection("users").doc(uid).get();
    const userData = userDoc.data() || {};
    const creatorName = `${userData.firstName || ""} ${userData.lastName || ""}`.trim() ||
...
// ============================================================
exports.deleteSecretV2 = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth)
        throw new functions.https.HttpsError("unauthenticated", "Login required");
    const uid = context.auth.uid;
    ...
    const userDoc = await db.collection("users").doc(uid).get();
    const isAdmin = ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.isAdmin) === true;
...
// ============================================================
exports.interactWithSecret = functions.https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError("unauthenticated", "Login required");
    const uid = context.auth.uid;
...
// ============================================================
exports.migrateSecretContent = functions.https.onCall(async (_data, context) => {
    var _a;
    if (!context.auth)
        throw new functions.https.HttpsError("unauthenticated", "Login required");
    // Admin only
    const userDoc = await db.collection("users").doc(context.auth.uid).get();
    if (((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.isAdmin) !== true) {
